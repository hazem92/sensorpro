...
long HH10D::getFrequency() {
  ...
  for(unsigned int j=0; j<SAMPLES; j++)  freq+= 500000.0/pulseIn(_pin, HIGH,250000);
  return freq / SAMPLES;
#else
    Serial.println("Using Interrupts.. ") ;
diff=(millis() - lastRead[_pin]);
    //Serial.println(diff) ;
  freq =(float) ticks[_pin]*1000 /diff;
 //Serial.println("ticks") ;

    //Serial.println(ticks[_pin]) ;
   // Serial.println("freq ") ;
    Serial.println(freq) ;
  lastRead[_pin] = millis();
  ticks[_pin] = 0;
